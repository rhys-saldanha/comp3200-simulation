...
class Type:
    ...
    def __init__(self, name: str, initial_size: float, birth_rate: float = 1.0, death_rate: float = 1.0):
        ...
        self.name = name
        self.rates = (birth_rate, death_rate)
        self.full_name = "{}({},{})".format(self.name, *self.rates)
    ...
    def rates_total(self) -> float:
        return sum(self.rates) * self.size

    def add_mutations(self, no_mutation: float, *args: Tuple['Type', float]):
        """
        Add mutation targets to a type of individual with probabilities. If the sum of probabilities is greater
        than 1 it will be normalised.
        ...
        self.mutations: List[Tuple['Type', float]]
        self.mutations = [(x[0], x[1] / s) for x in args]
